...
resource "aws_launch_template" "templete" {
  for_each = length(var.instance_count1)
 ...
 network_interfaces {
  ...
  tag_specifications {
    //for_each = toset(["instance", "jg"])
    count = length(var.desired_capacity)
    resource_type = "instance"

    tags = {
      Name = "${var.env}-Mintemir-${index.count}"
    }
  }
